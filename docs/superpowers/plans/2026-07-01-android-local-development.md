...
**Goal:** Make Mr Broccoli reliable to build and run on a local Android device from the existing Expo/React Native Android project.
...
Mr Broccoli already contains a native Android project under `android/`. Use this path for local device development; do not regenerate the native project unless an Expo upgrade requires it.
