...
class ColumboNativeAudioQueueRuntimeTest {
  @Test
  fun mediaPlayerBackedQueueEmitsStartedFinishedAndDrainedForLocalAudio() {
    ...
    val events = Collections.synchronizedList(mutableListOf<ColumboAudioQueueEvent>())
    val eventLatch = CountDownLatch(5)
    val coordinator = ColumboAudioQueueCoordinator(
      playerFactory = { item, callbacks ->
        ColumboAndroidAudioQueuePlayer(
          context = context,
          item = item,
          callbacks = callbacks,
        )
  ...
  fun outputWaveformPlaybackStateAdvancesAndStopsOnDeviceRuntime() {
    ...
    ColumboWaveformStateCoordinator.stopPlayback("output", "runtime-output")
    assertTrue(ColumboWaveformStateCoordinator.samples("output").all { it == 0.0 })
  }

  private fun item(id: String, uri: String) = ColumboAudioQueueItem(
    uri = uri,
    itemId = id,
