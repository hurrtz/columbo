...
final class MrBroccoliWaveformInputProcessor {
  ...
  func shape(samples: [Float]) -> [Float] {
    ...
    let shapedSamples = samples.map { sample -> Float in
      ...
      return MrBroccoliWaveformAudioAnalysis.clamp(sample: shapedMagnitude * inputVisualGain)
    }
    ...
    return shapedSamples.enumerated().map { index, sample in
      ...
      return MrBroccoliWaveformAudioAnalysis.clamp(
  ...
  func extractEnvelopeSamples(
    ...
    let samples = (0..<targetCount).map { index in
      ...
      for frameIndex in start..<end {
        let sample =
          (0..<channelCount).reduce(Float.zero) { partialResult, channelIndex in
            partialResult +
              MrBroccoliWaveformAudioAnalysis.sampleValue(
                in: buffer,
      ...
      return MrBroccoliWaveformAudioAnalysis.clamp(sample: nextEnvelopeLevel)
    }
