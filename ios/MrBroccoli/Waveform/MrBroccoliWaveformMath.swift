...
struct MrBroccoliWaveformLiveSnapshot {
  let samples: [CGFloat]
  let updatedAt: CFTimeInterval
  let appendedCount: Int
}

struct MrBroccoliWaveformPlaybackState {
  let itemId: String
  let samples: [CGFloat]
...
enum MrBroccoliWaveformMath {
  ...
  static func delayedLiveSamples(
    snapshots: [MrBroccoliWaveformLiveSnapshot]?,
  ...
  static func playbackWindow(
    playbackState: MrBroccoliWaveformPlaybackState,
    now: CFTimeInterval,
