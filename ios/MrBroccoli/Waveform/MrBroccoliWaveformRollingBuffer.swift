...
final class MrBroccoliWaveformRollingBuffer {
  private let sampleCount: Int
  private let stateLock = NSLock()
  private let inputProcessor: MrBroccoliWaveformInputProcessor
  ...
  init(sampleCount: Int, referenceFloor: Float) {
    ...
    self.inputProcessor = MrBroccoliWaveformInputProcessor(referenceFloor: referenceFloor)
  ...
  func reset() {
    ...
    MrBroccoliWaveformCoordinator.shared.clear(channel: .input)
  }
  ...
  func append(samples: [Float]) {
    ...
    for sample in shapedSamples {
      rollingSamples[rollingCursor] = MrBroccoliWaveformAudioAnalysis.clamp(sample: sample)
    ...
    MrBroccoliWaveformCoordinator.shared.setSamples(
      channel: .input,
