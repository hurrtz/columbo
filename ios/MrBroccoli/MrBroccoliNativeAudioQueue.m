...
static NSString *const MrBroccoliNativeAudioQueueEventName = @"MrBroccoliNativeAudioQueueEvent";

@interface MrBroccoliNativeAudioQueue : RCTEventEmitter <RCTBridgeModule>
@end

@implementation MrBroccoliNativeAudioQueue {
  MrBroccoliAudioQueueCoordinator *_coordinator;
  BOOL _hasListeners;
}
...
{
  if ((self = [super init])) {
    _coordinator = [MrBroccoliAudioQueueCoordinator new];
    ...
    _coordinator.eventHandler = ^(NSDictionary *payload) {
      ...
      [strongSelf sendEventWithName:MrBroccoliNativeAudioQueueEventName body:payload];
    };
  }
  return self;
}
...
{
  return @[ MrBroccoliNativeAudioQueueEventName ];
}
