...
static NSString *const ColumboNativeAudioQueueEventName = @"ColumboNativeAudioQueueEvent";

@interface ColumboNativeAudioQueue : RCTEventEmitter <RCTBridgeModule>
@end

@implementation ColumboNativeAudioQueue {
  ColumboAudioQueueCoordinator *_coordinator;
  BOOL _hasListeners;
}
...
{
  if ((self = [super init])) {
    _coordinator = [ColumboAudioQueueCoordinator new];
    ...
    _coordinator.eventHandler = ^(NSDictionary *payload) {
      ...
      [strongSelf sendEventWithName:ColumboNativeAudioQueueEventName body:payload];
    };
  }
  return self;
}
...
{
  return @[ ColumboNativeAudioQueueEventName ];
}
