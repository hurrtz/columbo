...
final class ColumboWaveformRollingBuffer {
  private let sampleCount: Int
  private let stateLock = NSLock()
  private let inputProcessor: ColumboWaveformInputProcessor
  ...
  init(sampleCount: Int, referenceFloor: Float) {
    ...
    self.inputProcessor = ColumboWaveformInputProcessor(referenceFloor: referenceFloor)
  ...
  func reset() {
    ...
    ColumboWaveformCoordinator.shared.clear(channel: .input)
  }
  ...
  func append(samples: [Float]) {
    ...
    for sample in shapedSamples {
      rollingSamples[rollingCursor] = ColumboWaveformAudioAnalysis.clamp(sample: sample)
    ...
    ColumboWaveformCoordinator.shared.setSamples(
      channel: .input,
