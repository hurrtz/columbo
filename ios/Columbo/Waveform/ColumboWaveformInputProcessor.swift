...
final class ColumboWaveformInputProcessor {
  ...
  func shape(samples: [Float]) -> [Float] {
    ...
    let shapedSamples = samples.map { sample -> Float in
      ...
      return ColumboWaveformAudioAnalysis.clamp(sample: shapedMagnitude * inputVisualGain)
    }
    ...
    return shapedSamples.enumerated().map { index, sample in
      ...
      return ColumboWaveformAudioAnalysis.clamp(
  ...
  func extractEnvelopeSamples(
    ...
    let samples = (0..<targetCount).map { index in
      ...
      for frameIndex in start..<end {
        let sample =
          (0..<channelCount).reduce(Float.zero) { partialResult, channelIndex in
            partialResult +
              ColumboWaveformAudioAnalysis.sampleValue(
                in: buffer,
      ...
      return ColumboWaveformAudioAnalysis.clamp(sample: nextEnvelopeLevel)
    }
