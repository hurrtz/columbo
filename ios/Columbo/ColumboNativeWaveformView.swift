...
final class ColumboWaveformView: UIView {
  @objc var channel: NSString = ColumboWaveformChannel.input.rawValue as NSString {
    didSet {
      waveformChannel =
        ColumboWaveformChannel(rawValue: channel as String) ?? .input
      updateLayerAppearance()
      redrawCurrentFrame()
    }
  }

  @objc var renderStyle: NSString = ColumboWaveformRenderStyle.automatic.rawValue as NSString {
    didSet {
      updateLayerAppearance()
      redrawCurrentFrame()
    }
  ...
  private var waveformChannel: ColumboWaveformChannel = .input
  private var displayLink: CADisplayLink?
  private var renderedSamples = Array(repeating: CGFloat.zero, count: 96)

  private var resolvedRenderStyle: ColumboWaveformRenderStyle {
    if let explicitStyle = ColumboWaveformRenderStyle(rawValue: renderStyle as String),
  ...
  @objc private func handleDisplayLink() {
    ...
    let sourceSamples = ColumboWaveformCoordinator.shared.samples(for: waveformChannel)
    let visuallyTunedSamples =
      resolvedRenderStyle == .envelope
        ? ColumboWaveformRendering.tuneEnvelopeSamples(sourceSamples)
        : ColumboWaveformRendering.tuneOutputSamples(sourceSamples)
    let pointCount =
      resolvedRenderStyle == .waveform
        ? max(32, min(92, Int(bounds.width / 3.2)))
        : max(28, min(82, Int(bounds.width / 3.6)))
    let spatiallySmoothed = ColumboWaveformRendering.smoothSpatialSamples(
      ColumboWaveformRendering.resample(samples: visuallyTunedSamples, count: pointCount),
  ...
  private func redrawCurrentFrame() {
    ...
    if resolvedRenderStyle == .envelope {
      ...
      fillLayer.path =
        active
          ? ColumboWaveformRendering.buildEnvelopeFillPath(
              bounds: bounds,
              samples: renderedSamples
            ).cgPath
          : nil
    ...
    let waveformPath = ColumboWaveformRendering.buildWaveformPath(
      bounds: bounds,
      samples: renderedSamples,
      channel: waveformChannel,
    ...
    fillLayer.path =
      active
        ? ColumboWaveformRendering.buildWaveformFillPath(
            bounds: bounds,
            samples: renderedSamples,
            channel: waveformChannel,
            displayScale: displayScale
...
@objc(ColumboNativeWaveformView)
final class ColumboNativeWaveformViewManager: RCTViewManager {
