...
@interface ColumboBackgroundVoiceTurn : NSObject <RCTBridgeModule>
@end

@interface ColumboBackgroundVoiceTurn ()
- (void)beginBackgroundTaskIfNeeded;
- (void)endBackgroundTask;
@end

@implementation ColumboBackgroundVoiceTurn {
  BOOL _turnActive;
...
- (void)beginBackgroundTaskIfNeeded
{
  ...
  _backgroundTask = [[UIApplication sharedApplication]
      beginBackgroundTaskWithName:@"Columbo voice turn"
                expirationHandler:^{
                  RCTLogWarn(@"Columbo voice turn background time expired.");
                  [weakSelf endBackgroundTask];
                }];

  if (_backgroundTask == UIBackgroundTaskInvalid) {
    RCTLogWarn(@"Columbo could not begin voice turn background time.");
  }
}
