...
#import "SchnackAudioQueueSession.h"

@interface SchnackAudioQueueCoordinator ()
...
@implementation SchnackAudioQueueCoordinator {
...
{
  [self ensurePlayer];
  return [SchnackAudioQueueSession activatePlaybackSession:error];
}
...
- (BOOL)stop:(NSError **)error
{
  @try {
    ...
    [SchnackAudioQueueSession deactivatePlaybackSessionIfIdleForPlayer:_player];
    return YES;
  } @catch (NSException *exception) {
    if (error != nil) {
      *error = [NSError errorWithDomain:@"SchnackAudioQueue"
...
- (void)handleItemDidPlayToEnd:(NSNotification *)notification
{
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    [self emitStartedForCurrentItemIfNeeded];
    [self emitDrainedIfNeeded];
    [SchnackAudioQueueSession deactivatePlaybackSessionIfIdleForPlayer:self->_player];
  });
}
